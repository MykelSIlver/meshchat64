...
; Ingang: A = aantal × 10ms (1..255), max = 2.55s
...
WS_HANDSHAKE
        ...
        ; 1000ms real via CIA2 — kloksnelheid-onafhankelijk (Bug 6 fix)
...
; Output:
;   AES_NONCE[0..11]   verse nonce (door GCM_NONCE_GEN gegenereerd)
...
AGD_K   LDA GCM_IN_KEY,X
        ...
        STA AES_CTR_LEN+1
        JSR AES_CTR_CRYPT
        ; -- optionele GCM-tag-verificatie (one-shot via GCM_DO_VERIFY) --
        LDA GCM_DO_VERIFY
        BNE AGD_VRF
...
;   AES_CTR_LEN        berichtlengte 1..255 bytes
...
AES_RK     !fill 240,$00        ; rondesleutels: 176 (AES-128) / 240 (AES-256)
...
AES_NONCE    !fill 12,$00       ; 12-byte nonce (stel in per bericht)
...
AES_CTR_LEN  !byte $00,$00      ; berichtlengte 16-bit (lo,hi)
...
GCM_H        !fill 16,$00       ; hash-subsleutel H = AES(key, 0^16)
...
GCM_MSG_CTR  !byte $00,$00  ; 16-bit berichtteller: garandeert nonce-uniciteit
...
; ================================================================
C64_IDBYTES  !fill 12,$00   ; eerste 12 bytes SHA-256(encryptiesleutel)
...
RCV_TOOLONG  !byte $00      ; 1 = data-array > RCV_CT (bericht te lang)
...
PRM_CNT      !byte $00      ; JSON_PARSE_ARR: verwacht aantal (0=variabel)
...
MSG_SHOWN    !byte $00      ; 1 = bericht getoond (trigger herteken)
...
B64_T        !byte $00      ; B64URL: tijdelijke combinatiebyte
...
SM_ID   lda #0
        ...
        sta ACC_Y          ; Y = 1
        sta ACC_Z          ; Z = 1
        ; ascending counter SM_CNT = 0..255 ; bit to test = 255 - SM_CNT.
        ; (Ascending counter, because 255 as start value is negative and breaks BPL.)
        lda #0
        sta SM_CNT
SM_LOOP
        ; SM_BIT = 255 - SM_CNT
        ...
        sbc SM_CNT
        sta SM_BIT
        ; --- ACC = 2*ACC ---  (copy ACC->A-coords, dbl, R->ACC)
        +fcopy AX, ACC_X
        +fcopy AY, ACC_Y
        +fcopy AZ, ACC_Z
        +fcopy AT, ACC_T
        ...
        +fcopy ACC_Z, RZ
        +fcopy ACC_T, RT
        ; --- test bit SM_BIT of SCALAR ---
        jsr SM_TESTBIT     ; C = bit
        bcs SM_DOADD       ; bit set -> conditional addition
        jmp SM_NEXT        ; bit 0 -> skip (JMP due to distance)
SM_DOADD
        ; --- ACC = ACC + PIN --- (A=ACC, B=PIN, R->ACC)
        +fcopy AX, ACC_X
        +fcopy AY, ACC_Y
        +fcopy AZ, ACC_Z
        +fcopy AT, ACC_T
...
SM_NEXT
        inc SM_CNT
        lda SM_CNT         ; set Z flag; after 256 iterations (0..255) wraps to 0
        beq SM_DONE        ; done when all 256 bits processed
        jmp SM_LOOP        ; else continue (JMP due to large loop body)
SM_DONE
        RTS

; ================================================================
; SCALAR_MUL_N : like SCALAR_MUL but processes only the lowest
; SM_NBITS bits (from bit SM_NBITS-1 down to 0). Identical loop logic;
; meant to test double-and-add with multiple ADDs FAST in py65.
; ================================================================
SCALAR_MUL_N
        ldx #31
SN_ID   lda #0
        ...
        sta ACC_Y
        sta ACC_Z
        ; SM_BIT = SM_NBITS-1 (start), count down below 0
        lda SM_NBITS
        sec
        sbc #1
        sta SM_BIT
...
SN_NEXT
        dec SM_BIT
        bmi SN_DONE        ; SM_BIT < 0 -> done (NBITS <= 128, no wrap risk)
        jmp SN_LOOP
SN_DONE
        RTS

; ----------------------------------------------------------------
; SM_TESTBIT : bit index in SM_BIT (0..255) of SCALAR -> carry
;   byte = SCALAR[SM_BIT>>3] ; bit = SM_BIT & 7
; ----------------------------------------------------------------
SM_TESTBIT
        lda SM_BIT
        lsr
...
FP_SQ_IP
        ...
        JMP FP_MUL              ; tail-call; FP_MUL's RTS returns to SQN_L
...
; ================================================================
; WS_SEND_MSG256 - 256-bit bericht: AES-256-GCM + Ed25519-sig (Optie B)
...
WSM2_K  LDA KEY_PEER256,X
        ...
        ; -- verse nonce + AES-256-GCM encrypt --
        JSR GCM_NONCE_GEN
...
SB_O2   LDA B64E_OUT,X
        ...
        LDA #$2E
        STA SHARE_STR+87
        ; --- segment 3: relayWss = btoa(wssUrl), voorberekende base64 ---
        LDX #43
